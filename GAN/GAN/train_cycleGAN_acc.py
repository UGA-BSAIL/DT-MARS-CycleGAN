...
def train(opt, accelerator):
    ...
    netG_A2B, netG_B2A, netD_A, netD_B, optimizer_G, optimizer_D_A, optimizer_D_B, dataloader = accelerator.prepare(
        netG_A2B, netG_B2A, netD_A, netD_B, optimizer_G, optimizer_D_A, optimizer_D_B, dataloader
    )
    ...
    ###### Training ######
    step = 0
    for epoch in tqdm(range(opt.epoch, opt.n_epochs), total=opt.n_epochs):
        for i, batch in enumerate(dataloader):
            ...
            if (step)%10 == 0:
                ...
                # accelerator.print(loss_G, loss_identity_A + loss_identity_B, loss_GAN_A2B + loss_GAN_B2A, loss_cycle_ABA + loss_cycle_BAB, loss_D_A + loss_D_B)
            if accelerator.is_main_process:
        ...
        if (epoch+1)%5 == 0 and accelerator.is_main_process:
            ...
            accelerator.save(netG_A2B.module.state_dict(), '{}/netG_cycle_A2B_withdet_{}.pth'.format(opt.outdir, epoch+1))
            accelerator.save(netG_B2A.module.state_dict(), '{}/netG_cycle_B2A_withdet_{}.pth'.format(opt.outdir, epoch+1))
            accelerator.save(netD_A.module.state_dict(), '{}/netD_cycle_A_withdet_{}.pth'.format(opt.outdir, epoch+1))
            accelerator.save(netD_B.module.state_dict(), '{}/netD__cycle_B_withdet_{}.pth'.format(opt.outdir, epoch+1))
