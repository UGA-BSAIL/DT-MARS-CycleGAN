...
def train(opt, accelerator):
    ...
    netG_A2B, netG_B2A, netD_A, netD_B, netDet, optimizer_G, optimizer_D_A, optimizer_D_B, dataloader = accelerator.prepare(
        netG_A2B, netG_B2A, netD_A, netD_B, netDet, optimizer_G, optimizer_D_A, optimizer_D_B, dataloader
    )

    # Loss plot
    if accelerator.is_main_process:
        writer = SummaryWriter('./runs/RetinaGAN')
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        for i, batch in enumerate(dataloader):
            ...
            if (step)%10 == 0:
                ...
                # accelerator.print(loss_G, loss_identity_A + loss_identity_B, loss_GAN_A2B + loss_GAN_B2A, loss_cycle_ABA + loss_cycle_BAB, loss_D_A + loss_D_B, loss_consist)
            if accelerator.is_main_process:
